...
| Executable formal model | A deterministic state-transition specification explored by a checker to find invariant violations. It is not a language model, generates no operational decision, and proves no runtime implementation correct. |
...
For the limited C4 profile, freshness is decision-relative rather than a TTL-only property. Observation time, receipt time, use time, declared clock uncertainty, exact target UID, and every decision-critical resource version are distinct inputs. An unexpired TTL does not establish freshness after a decision-critical version changes. The assumptions and claim boundary are specified in [Appendix G](#appendix-g-limited-c4-human-approved-apply-profile-100); the executable relations define Evidence freshness at consumer use (`EVD-R09`), propagation of invalidation after a critical version changes (`EVD-R14`), and delivery of that freshness fact to the exact C4 decision (`CEI-R02`).
...
The limited C4 contract has the stable profile identifier `agent-ops.c4-human-approved-apply@1.0.0`. Under this profile, a human decision approves one canonical change package and a human separately starts its deterministic Executor. Approval alone is not backend admission, does not prove an effect, and does not authorize a package variant. Appendix G fixes the scope and assumptions; the executable relations define exact package binding (`C4-R01`), current approval at admission (`C4-R02`), a current authenticated human start (`C4-R03`), and authorized admission at the declared atomic point (`C4-R06`).
...
The initial reference scope of `agent-ops.c4-human-approved-apply@1.0.0` is limited to predeclared typed operations against an isolated non-production test target. Arbitrary shell execution, standing C5 authority, irreversible production changes, and arbitrary third-party APIs without declared admission, identity, retry, observation, and fencing semantics are outside the profile. A pre-call check against a non-cooperating remote backend MUST NOT be described as atomic authorization of the remote effect.
...
The existing `foundation_evidence_bundle` v1 contract retains its local meaning in this edition. It may carry references produced under the Evidence Acquisition profile, but its digest, sealing, or schema validity does not by itself establish source authority, acquisition validity, coverage, freshness, independence, claim support, or execution authority. A stronger bundle contract, if required after the formal relation specifications are accepted, will use a new incompatible schema identity or major version with an explicit migration note.
...
For the limited C4 profile, the inventory also records the exact versions and configuration identities of every trusted computing base component named in Appendix G, including the canonicalizer, identity and policy authority, signer and key store, authoritative log, reservation store, gateway or cooperating backend adapter, effect observer, Outcome oracle, and time and version sources. Shared credentials, models, evidence sources, canonicalizers, clocks, control-plane services, and observation paths are declared as common-cause dependencies rather than counted as independent controls.
...
For the limited C4 profile, Guardian independence requires separation of authority, credentials, evidence sources, and observation paths. A second LLM, another prompt, or a second process using the same authority and evidence path is not sufficient independence. The Guardian cannot mint approval, hold target-write credentials, or repair an evaluated record through an unrecorded action.
...
Deterministic validation and a model-assisted semantic review MAY share inputs, but any shared model, evidence source, canonicalizer, clock, credential, policy service, or control-plane dependency is one common-cause boundary. Claims of independent detection are limited to the components and paths that remain independent after those dependencies are enumerated.
...
The limited C4 trust boundary is cataloged in Appendix G. Admission safety, knowledge of an effect, Outcome success, and later recovery are separate claims. A safe stop establishes none of the other three, and no atomic remote-authorization claim is made unless a cooperating gateway or backend enforces the decision at its declared admission point.
...
A checkpoint is a derived summary, never the authority source for C4. It may refine only the authoritative durable log and reservation state: it cannot delete a consumed attempt, admitted or unresolved effect, pending reconciliation or compensation, current approval or policy epoch, reservation, fence, or observation-completeness limitation. The checkpoint-refinement relation (`C4-R12`) fixes this rule and retains the assumption-removal and combined omission counterexamples listed in Appendix G.
...
| evidence_refs | Verifiable references with source, timestamp, freshness, authority, and digest. A reference proves neither that its acquisition was valid nor that it is sufficient for the consuming claim unless the applicable evidence requirement is evaluated. |
...
## Appendix G. Limited C4 Human-approved Apply Profile 1.0.0
...
The initial reference domain contains only predeclared typed operations against an isolated, resettable, non-production test target. Every operation declares target identity, version semantics, admission semantics, idempotency or reconciliation behavior, conflict domain, fencing support, observation coverage, and Outcome oracle. A deployment MAY implement a narrower subset, but MUST NOT silently broaden it.

The following are outside profile 1.0.0:

- standing C5 authority or approval created automatically for a human;
- arbitrary shell commands, unconstrained scripts, or undeclared third-party APIs;
- production or irreversible effects, and general isolation from non-cooperating writers;
- exactly-once claims for an arbitrary external effect;
- recovery claims while the required control plane, observer, or Outcome oracle remains unavailable;
- claims that schema validation alone establishes cross-record authority, temporal safety, implementation safety, scientific novelty, or production readiness.

### G.2. Guarantee vocabulary and claim rules

The following terms are not interchangeable:

| Term | Controlled meaning |
| --- | --- |
| admission safety | No operation is admitted at the declared enforcement point unless the profile's authority predicate holds for that exact package, target, attempt, policy epoch, approval epoch, and use time. |
| effect knowledge | The classified knowledge `applied`, `not_applied`, `partial`, or `unknown` about target changes attributable to an attempt, accompanied by observer coverage. It is not the attempt state or Outcome. |
| Outcome success | The canonical Outcome oracle reports `succeeded` against the original Intent and bound after-state evidence. An admitted or applied operation need not succeed. |
| recovery | Every admitted unresolved obligation eventually reaches an authorized terminal disposition and required service or test-state restoration under stated availability and reversibility assumptions. A safe stop is not recovery. |
| retry safety | A repeated delivery cannot create an effect outside the operation's declared semantics because the same backend/idempotency key is enforced, complete observation proves no prior effect, or authorized reconciliation determines the next action. Compensation alone is not retry safety. |
| conflict safety | Cooperating writers that overlap any target write or decision-critical read are serialized or rejected within the declared enforcement domain, and stale writers are fenced. |
| continuity | Attempts, reservations, fences, approval and policy epochs, and unresolved effects survive checkpoint, handoff, restart, and lease expiry without reset or omission. |
| evidence completeness | The observer explicitly records what it covered, excluded, or could not determine. No observed change under incomplete coverage does not mean `not_applied`. |
...
This appendix establishes the vocabulary and assumptions below. G.7 supplies the bounded executable definitions; they become claimable runtime guarantees only after the exact candidate and its conformance obligations are accepted and the exact implementation supplies current evidence.

### G.3. Assumption catalog

Each assumption has a stable identifier. Implementations MUST declare `satisfied`, `not_satisfied`, or `unknown` with evidence for every applicable assumption; only `satisfied` supports a dependent claim.

| ID | Assumption and required evidence | Claim consequence when removed, failed, or unknown |
| --- | --- | --- |
| C4-A01 | The target is isolated, resettable, non-production, and admits only the registered typed operation set. Evidence identifies the target class and reset procedure. | Profile applicability and recovery evaluation are not established. |
| C4-A02 | Every writer that can overlap a protected write or decision-critical read cooperates with the declared gateway, reservation, and fencing domain. Out-of-domain writers are enumerated and observed. | Admission and conflict safety apply only inside the cooperating domain; an outside writer can invalidate them. |
| C4-A03 | The gateway or backend adapter has a documented atomic admission point that checks the bound authority predicate before accepting the mutation. | A pre-call check provides local policy evidence only; no atomic remote-authorization claim is permitted. |
| C4-A04 | The backend supplies immutable target UIDs and monotonic or otherwise ABA-resistant decision-critical resource versions. | Name reuse or ABA can redirect an approval or make stale evidence appear current. |
| C4-A05 | Each operation declares backend idempotency, same-key replay, partial-effect, cancellation, reconciliation, and fencing semantics. | Retry, cancellation, and stale-writer safety cannot be inferred. |
| C4-A06 | The authoritative lifecycle log is append-only, durably ordered, and atomic for each accepted event; corruption and unavailability are detectable. | Event absence, order, attempt consumption, and unresolved obligations cannot be trusted. |
| C4-A07 | The reservation store provides durable atomic acquisition/update and monotonic fencing tokens over the declared conflict domain. | Concurrent or expired writers can both act; conflict safety and continuity fail. |
| C4-A08 | Principal identity, signer identity, policy identity, and key lifecycle are authenticated; signing keys are protected and revocation is visible at use. | Package or approval provenance and authority validity are not established. |
| C4-A09 | Planner, Guardian, observer, and ordinary users lack target-write credentials; Executor credentials are operation-, target-, time-, and attempt-scoped. | A bypass path can create an unattributed or unauthorized effect. |
| C4-A10 | Time sources, validity-bound interpretation, and maximum clock uncertainty are declared. Admission treats the earliest possible expiry and latest possible `not_before` conservatively. | An apparently valid proof can be expired or not yet valid at the backend admission point. |
| C4-A11 | The read-only effect observer declares resource, field, time, and external-effect coverage and reports `complete`, `partial`, or `unknown`. It is operationally separate from the Executor's response path. | Absence of an observed change cannot establish `not_applied`; effect knowledge remains `unknown`. |
| C4-A12 | The canonical Outcome oracle is bound to the original Intent, target UID, after-state evidence, and completion rules; its independence from the operation path is declared. | Effect knowledge cannot be promoted to Outcome success, and service recovery cannot be claimed. |
| C4-A13 | External controllers, asynchronous reconcilers, admission webhooks, and backend side effects are enumerated with their writer and observation domains. | Attribution, conflict classification, and resulting-state claims exclude undeclared controllers. |
| C4-A14 | Availability assumptions are explicit for the identity/policy service, log, reservation store, gateway/backend, observer, and Outcome oracle. Loss of a required service causes fail-closed admission where enforceable. | Safety may degrade to a stop; eventual completion, reconciliation, and recovery are not guaranteed during unavailability. |
| C4-A15 | Irreversible effects are excluded. Every admitted operation has a declared reset or separately authorized compensation/reconciliation path whose residual effects are observable. | Restoration and bounded residual-risk claims are invalid. |
| C4-A16 | Credential, signing-key, gateway/backend, log, reservation-store, observer, or oracle compromise is outside the corresponding component's guarantee and is separately detected and handled. | A compromised TCB component can forge or suppress the fact it is trusted to enforce or observe. |
| C4-A17 | Checkpoints and summaries are derived from, and can only refine, authoritative durable log and reservation state. Resume reads the authoritative state before proceeding. | Restart or handoff can reset attempts, epochs, reservations, fences, or unresolved obligations. |
| C4-A18 | Canonicalization and digest algorithms are versioned, deterministic, collision-resistant for the profile, and identically interpreted by approval and admission components. | Different packages can share or appear to share an approval identity. |
| C4-A19 | The approving human and starting human are authenticated, are shown the canonical package identity and declared consequences, and act under an applicable organizational authority rule. | A valid signature or UI event does not establish accountable human authority for the operation. |

### G.4. Trusted computing base

Planner and model output are untrusted proposal inputs and are not in the admission TCB. Guardian is not trusted to create authority; it is trusted only for the exact independent checks whose inputs and paths satisfy the table. Every implementation MUST publish an equivalent deployment-specific table.

| Component | Enforcement or observation boundary | Failure or compromise assumption | Required independence | Limit on claims |
| --- | --- | --- | --- | --- |
| Identity and policy authority | Authenticates principals and supplies current organizational authority, policy epoch, revocation, and scope. | Availability is declared; forged identity, stale epoch, or compromise invalidates authority claims. | Separate from Planner and target-write path; revocation checked at use. | Cannot prove package integrity, backend admission, effect, or Outcome. |
| Signer and key store | Signs the exact canonical human decision or attestation and protects signing material. | Key extraction, unauthorized signing, or hidden revocation invalidates provenance. | Approval key is unavailable to Planner, Guardian, and Executor. | Signature proves origin and integrity, not safety or backend use. |
| Canonicalizer and digest implementation | Produces the byte identity approved and later checked at admission. | Version skew, ambiguity, or collision invalidates package binding. | Approval and admission may use separate instances but MUST use the same accepted algorithm/version and test vectors. | Cannot establish authority, freshness, or effect. |
| Authoritative durable log | Orders immutable lifecycle facts, attempts, admissions, effect classifications, receipts, reservations, reconciliation, compensation, and checkpoint lineage. | Accepted-event atomicity and durability are assumed; detectable unavailability fails closed where required. | Writer identities are authenticated; observer and Executor cannot rewrite history. | Logged assertion is not proof of an external effect unless backed by the declared observer. |
| Reservation and fence store | Serializes declared conflict domains and issues monotonic fence tokens. | Atomic durable update and backend fence enforcement are assumed. | Executor cannot reset epochs or mint an accepted fence outside policy. | Protects only cooperating writers and declared read/write conflicts. |
| Gateway or cooperating backend adapter | Checks authority at use and admits/rejects the typed operation at the declared linearization point; enforces version preconditions, key, and fence. | Its admission semantics are trusted; compromise or a bypass writer invalidates admission safety. | No Planner/Guardian credential; policy and package identities are independently verifiable. | A separate pre-call check cannot substitute for this boundary. |
| Effect observer | Reads target and declared external surfaces, attributes changes, and reports coverage and `applied` / `not_applied` / `partial` / `unknown`. | Coverage gaps and unavailability are explicit; compromise can forge effect knowledge. | Read-only credentials and an observation path independent of the Executor response. | Cannot grant authority or by itself decide Outcome. |
| Workload and Outcome oracle | Evaluates the original Intent against canonical after-state and service/test evidence. | Oracle correctness and declared coverage are assumed; unavailable or conflicting evidence yields `unknown`. | Separate from Planner and mutation response; common data sources are disclosed. | Outcome success does not prove admission safety or absence of residual effects. |
| Human approver and human starter | Supplies accountable approval and a separate start event for the exact package. | Authenticated identity, comprehensible canonical display, and organizational authority are assumed. | Cannot be replaced by an agent, metric, or Guardian result; applicable self-approval rules still apply. | Human action cannot repair stale evidence, package mismatch, backend bypass, or absent observation. |
| Time and version sources | Supply bounded time uncertainty, target UID, policy/approval epochs, and decision-critical versions used at admission. | Bounds, monotonicity or ABA resistance, and availability are declared. | Sources are authoritative for their named facts; shared sources are common-cause dependencies. | TTL alone never proves decision-relative freshness. |

### G.5. Common-cause failures and Guardian independence

The deployment record MUST enumerate shared language models, prompts, evidence producers, credentials, keys, canonicalizers, clocks, version services, policy and identity services, logs, storage systems, network paths, target APIs, observers, and Outcome data sources. Two checks that depend on the same failing fact are not independent evidence for that fact. Diversity of language-model name, prompt wording, process, or host MAY improve detection but does not establish independence unless authority, credentials, sources, and observation paths are also separated for the claimed failure class.

Guardian MAY block, redirect, or request separately authorized remediation. It MUST NOT approve on behalf of a human, mint or widen an Executor credential, modify the target, rewrite the authoritative record, conceal `unknown`, or repair the evaluated artifact outside the logged lifecycle. A common false proof supplied to both Planner and Guardian is retained as an executable counterexample and remains a required negative conformance fixture.

### G.6. Claim matrix

| Claim | Kind | Required assumptions and trusted components | Enforcement domain and present status |
| --- | --- | --- | --- |
| C4-G01 Exact-package admission safety | safety | C4-A02-A04, C4-A08-A10, C4-A18-A19; identity/policy authority, signer, canonicalizer, gateway/backend, time/version sources | Cooperating gateway/backend and exact typed operation only; bounded by exact package binding, current approval, current human start, held reservation, current fence, and authorized admission (`C4-R01`-`C4-R06`) plus P1/P2/P6; not yet a runtime claim. |
| C4-G02 Conflict and stale-writer safety | safety | C4-A02, C4-A04-A07, C4-A13; reservation/fence store and cooperating backend | Declared conflict domain and cooperating writers only; bounded by held reservation (`C4-R04`), current fence (`C4-R05`), and P4. |
| C4-G03 Retry safety | safety | C4-A05-A07, C4-A11, C4-A15; backend, log, observer, and authorized reconciler | Registered operation family only; safe retry (`C4-R08`) requires a backend key, complete negative Evidence, or authorized reconciliation; compensation alone is insufficient. |
| C4-G04 Authority and budget continuity | safety | C4-A06-A10, C4-A17-A18; log, reservation store, identity/policy, canonicalizer, time/version sources | Checkpoint, handoff, restart, and lease-expiry transitions; bounded by a monotonic attempt budget (`C4-R07`), checkpoint refinement (`C4-R12`), and P3. |
| C4-G05 Honest effect knowledge | epistemic safety | C4-A06, C4-A11, C4-A13; log and effect observer | Declared observation surface only; supported effect knowledge (`C4-R09`) and the applicability, freshness, coverage, disposition, sufficiency, provenance, and common-cause interface relations (`CEI-R01`-`CEI-R07`) require incomplete coverage to remain `unknown`; bounded by P5. |
| C4-G06 Outcome classification integrity | epistemic safety | C4-A06, C4-A12-A13; log and Outcome oracle | Original Intent and declared oracle coverage only; bounded by canonical Outcome classification (`C4-R10`) and P7. |
| C4-G07 Eventual obligation resolution | liveness | C4-A01, C4-A05-A07, C4-A11-A15, C4-A17; all required control-plane services and human escalation path | Resolved obligation (`C4-R11`), allowed terminal release (`C4-R13`), and L1 check an authorized bounded resolution path under availability and reversibility; unbounded liveness is not proved. |

### G.7. Executable formal state-transition models and relation catalog

The executable specification defines two compositional, versioned formal state-transition models: the C4 authority-and-effect model in `formal/c4.go` and the Evidence Acquisition assertion-and-sufficiency model in `formal/evidence.go`. These are deterministic specifications explored by the checker, not language models. `agent-ops.c4-evidence-interface@1.0.0` connects them without merging their authority domains. The independently versioned C4 catalog is `agent-ops.c4-relations@1.0.0`; every relation names its authority, check time, and failure disposition. The C4 state includes canonical package digest and version, target UID and decision-critical versions, approval and policy epochs, bounded time interval, human approval and start events, attempt budget and consumed attempts, conflict domain, reservation and fence, admission state, effect knowledge, Outcome state, reconciliation or compensation obligations, and checkpoint lineage.

Relation identifiers are precise normative locators, not reader-facing explanations. On first use, prose gives the readable name and meaning before the identifier; later compact references may use the identifier alone. The following index covers all three relation catalogs.

| ID | Reader-facing and catalog name | Meaning |
| --- | --- | --- |
| `C4-R01` | Exact package binding (`package_bound`) | Binds approval and admission to one canonical package, exact target, and decision-critical versions. |
| `C4-R02` | Current approval (`approval_current`) | Requires the signed human decision, identity, policy, and revocation state to be current at admission. |
| `C4-R03` | Current human start (`human_start_current`) | Requires an authenticated human-start event before the first attempt is admitted. |
| `C4-R04` | Held reservation (`reservation_held`) | Requires the authoritative reservation for the declared conflict domain at reservation and admission. |
| `C4-R05` | Current fence (`fence_current`) | Requires the cooperating backend to reject a stale writer at mutation. |
| `C4-R06` | Authorized admission (`admission_authorized`) | Requires the cooperating gateway or backend adapter to authorize the operation at the declared atomic admission point. |
| `C4-R07` | Monotonic attempt budget (`attempt_budget_monotonic`) | Prevents checkpoint or restart from resetting consumed attempts in the durable lifecycle log. |
| `C4-R08` | Safe retry (`retry_safe`) | Allows retry only through declared same-key semantics, complete applicable negative Evidence, or authorized reconciliation. |
| `C4-R09` | Supported effect knowledge (`effect_knowledge_supported`) | Requires effect classification to be supported by the declared observer's Evidence; otherwise the effect remains `unknown`. |
| `C4-R10` | Classified Outcome (`outcome_classified`) | Requires the canonical Outcome oracle and applicable Evidence; otherwise Outcome remains `unknown`. |
| `C4-R11` | Resolved obligation (`obligation_resolved`) | Requires an admitted unresolved effect to reach an authorized reconciliation or compensation disposition. |
| `C4-R12` | Checkpoint refinement (`checkpoint_refines`) | Allows a checkpoint only to refine, never reset or omit, authoritative log and reservation state. |
| `C4-R13` | Allowed terminal release (`terminal_release_allowed`) | Releases a case only when its authoritative lifecycle state permits it. |

| ID | Reader-facing and catalog name | Meaning |
| --- | --- | --- |
| `EVD-R01` | Applicable requirement (`requirement_applicable`) | Establishes whether a named consumer requirement applies to collection and use. |
| `EVD-R02` | Authorized collection profile (`collection_profile_authorized`) | Requires an authorized versioned collection profile before each attempt. |
| `EVD-R03` | Append-only attempts (`attempt_append_only`) | Preserves every attempt and failure through retry, checkpoint, and restart. |
| `EVD-R04` | Bound receipt (`receipt_bound`) | Binds a collection receipt to the exact target, source, and receipt event. |
| `EVD-R05` | Artifact integrity (`artifact_integrity`) | Verifies canonical bytes and digest before artifact admission or consumer use. |
| `EVD-R06` | Complete derivation lineage (`derivation_lineage_complete`) | Preserves provenance through every transformation that produces an artifact or assertion. |
| `EVD-R07` | Assertion disposition (`assertion_disposition`) | Records whether the bounded evaluator supports, refutes, does not test, or cannot determine an assertion. |
| `EVD-R08` | Valid negative observation (`negative_observation_valid`) | Accepts a negative assertion only with complete applicable coverage and detection capability. |
| `EVD-R09` | Fresh at use (`fresh_at_use`) | Evaluates exact target/source identity, time, and critical versions at the consumer decision. |
| `EVD-R10` | Supported independence (`independence_supported`) | Treats evidence as independent only when declared common-cause dependencies support that claim. |
| `EVD-R11` | Computed completion (`completion_computed`) | Computes bundle completion from the consumer requirement and complete acquisition lineage. |
| `EVD-R12` | Sealed bundle (`bundle_sealed`) | Permits sealing only over the computed completion state. |
| `EVD-R13` | Consumer-specific sufficiency (`consumer_sufficient`) | Evaluates sufficiency against the named consumer rule version at its decision. |
| `EVD-R14` | Propagated invalidation (`invalidation_propagated`) | Makes dependent facts `unknown` after a relevant target, source, profile, or policy version changes. |

| ID | Reader-facing and catalog name | Meaning |
| --- | --- | --- |
| `CEI-R01` | Applicable Evidence (`evidence_applicable`) | Carries requirement applicability to the exact C4 admission, effect, or Outcome decision. |
| `CEI-R02` | Evidence fresh at use (`evidence_fresh_at_use`) | Carries the Evidence freshness result to the exact C4 consumer decision. |
| `CEI-R03` | Observation coverage (`observation_coverage`) | Carries covered surfaces and detection limits into effect classification. |
| `CEI-R04` | Assertion disposition (`assertion_disposition`) | Carries the evaluator's bounded assertion disposition into effect or Outcome classification. |
| `CEI-R05` | Sufficient Evidence (`evidence_sufficient`) | Carries consumer-specific sufficiency; a failed or unknown result blocks the decision. |
| `CEI-R06` | Bound provenance (`provenance_bound`) | Binds each interface fact to its acquisition and derivation lineage. |
| `CEI-R07` | Disclosed common causes (`common_causes_disclosed`) | Carries shared dependencies that limit independence and sufficiency claims. |

Formal-model actions include package creation, human approval, revocation, human start, reservation, admission or denial at the gateway/backend linearization point, effect observation, response loss, retry request, lease expiry, fence rejection, reconciliation, compensation, Outcome evaluation, checkpoint, restart, and terminal release. Each state-transition model separates safety from bounded resolution-path analysis, represents unavailable or removed assumptions explicitly, uses named events rather than unbounded wall-clock waits, and preserves `unknown` as a first-class state.

Each proposed property requires at least one counterexample produced by removing a named assumption:

| Planned property | Named assumption removal | Required counterexample shape |
| --- | --- | --- |
| P1 No admission without exact current authority | remove C4-A02 or C4-A03 | A bypass writer mutates the target, or a remote backend accepts after a non-atomic pre-call check despite revocation. |
| P2 Approval binds one canonical package and exact target generation | remove C4-A04 or C4-A18 | Target deletion/recreation or canonicalizer disagreement lets a different UID or argument set reuse approval. |
| P3 Attempt budget and unresolved obligations are monotonic | remove C4-A06 or C4-A17 | Checkpoint/restart omits a consumed attempt or admitted unresolved effect and permits another attempt. |
| P4 Conflicting stale writers cannot both be effective | remove C4-A05 or C4-A07 | A lease expires without backend fencing and the old writer acts after a new writer is admitted. |
| P5 `not_applied` is emitted only with complete applicable observation | remove C4-A11 or C4-A13 | An unobserved external controller or side effect exists while the observer reports no change. |
| P6 Time-invalid authority is not admitted | remove C4-A10 | Clock skew exceeds the declared bound and an expired or not-yet-valid decision is accepted. |
...
### H.2. Controlled acquisition chain
...
| Object | Meaning in this profile |
| --- | --- |
| `EvidenceRequirement` | States the consumer, intended claim or decision, exact target and epoch, required sources, time and coverage bounds, authority, independence, freshness, negative-observation semantics, and the result of missing or conflicting evidence. |
| `CollectionProfile` | Versioned and digest-bound collector contract: tool and configuration, permissions and read-only boundary, source and target binding, population or sampling and seed, limits, retry and error rules, parser, normalization, redaction, raw references, coverage, common-cause dependencies, and reproducibility inputs. |
...
### H.3. Independent quality axes and claim rule

| Quality axis | Required question |
| --- | --- |
| Authority | Is the source recognized for this exact fact, scope, and time? |
| Identity and binding | Are consumer, target, source, epoch, and artifact unambiguously bound? |
| Integrity and provenance | Can the examined bytes and every transformation be traced without an unexplained gap? |
| Acquisition validity | Did the declared collector, query, permissions, bounds, parser, and error rules actually govern the attempt? |
| Coverage and completeness | What population, interval, surfaces, inclusions, exclusions, truncation, and dropped data were actually covered? |
| Freshness | Is the evidence fit at use time for the target and every decision-critical version? |
| Transformation fidelity | Did parsing, normalization, aggregation, redaction, or projection preserve the claim-relevant meaning and limitations? |
| Independence | Which sources, credentials, collectors, parsers, stores, clocks, paths, language models, or prompts are shared? |
| Claim support | Does the assertion support or refute the exact claim, merely fail to test it, conflict, or remain unknown? |
| Reproducibility | Can a reviewer recover the inputs, profile, receipts, artifacts, transformations, and bounded result? |

The consumer evaluates sufficiency against its own `EvidenceRequirement`; evidence is not globally sufficient. Every required axis MUST be explicit. Missing, conflicted, failed, or unverified blocking axes yield `unknown` or incomplete. An aggregate score MUST NOT compensate for failed authority, identity binding, integrity, or blocking coverage. Evidence may justify a factual claim only within the evaluated boundary; it MUST NOT grant approval, execution authority, or source authority.

### H.4. Trusted computing base

| Component | Required responsibility and claim limit |
| --- | --- |
| Source authority | Establishes which source may speak for which fact; does not prove the source correct. |
| Collector or tool | Executes the bound read contract and exposes failures and limits; successful transport does not prove completeness or truth. |
| Target/source identity service | Resolves exact identities and epochs; aliases alone do not prove binding. |
| Time/version source | Supplies bounded time and decision-critical versions; shared or uncertain sources limit freshness claims. |
| Raw evidence store | Retains exact artifacts or protected references; storage does not create authority. |
...
Transport failure, permission denial, timeout, parser failure, truncation, sampling shortfall, redaction loss, source conflict, and target or version mismatch are distinct outcomes and MUST remain recorded. A successful request or non-empty artifact does not erase them. A retry is another attempt with another receipt; it does not rewrite the prior attempt.

`not_observed` is evidence of absence only when the requirement defines the population and window and the receipt demonstrates complete applicable coverage and detection capability. Otherwise the assertion is `does_not_test` or `unknown`, not a negative fact. Conflicting authoritative sources remain conflicted until a declared resolution rule is applied; selection by convenience is not resolution.

### H.6. Interface with the limited C4 profile
...
### H.9. Integrated conformance and pre-runtime evaluation protocol

The composed offline profile `agent-ops.c4-evidence-integrated@1.0.0` pins the exact C4 and Evidence validators and the C4-Evidence interface. Its result reports local syntax, C4 relation validity, Evidence relation validity, consumer-specific Evidence sufficiency, cross-boundary validity, and the consuming decision separately. `allowed` requires every component result and the interface projection to pass; malformed input remains `unknown`, while a complete but invalid input is `blocked`. The composed checker cannot change a component relation or reason code, and an aggregate score cannot override a blocking finding.

The cross-boundary corpus pairs each negative trace with a valid baseline. It covers an Executor response used as effect proof, `not_applied` under partial coverage, Outcome success from stale or insufficient Evidence, retry from an incomplete negative observation, and checkpoint plus supersession hiding acquisition failure and an unresolved effect. Property tests verify order invariance, monotonic propagation of `unknown`, malformed-input safety, and exact catalog pins. The C4 `consumer_rule` fields use the same explicit versioned identifier syntax as Evidence Acquisition; this corrects the earlier non-composable candidate constraint and changes the pinned bytes of the two affected C4 schemas without changing their resource identities.

The pre-runtime evaluation and reproducibility protocol `agent-ops.c4-evidence-evaluation@1.0.0` freezes all assigned cases, deterministic seed `30404`, named barriers, exact tool/model/prompt/profile/schema/catalog versions, target and source snapshots, authority and expected-coverage maps, failure injection, reset and adjudication records, and the required raw and derived artifacts. `succeeded`, `failed`, `timeout`, `partial`, and `unknown` all remain in the denominator. Primary metrics separately cover unauthorized or duplicate effects, clean and adversarial task success, checkpoint violations, false completeness, unsupported assertions, wrong target or source, stale or hidden-loss Evidence, broken lineage, false independence, invalid negative evidence, and snapshot/method reproducibility.

The protocol records how a future separately authorized evaluation MUST be measured before results are inspected. It reports no collector or Executor run, pilot, deployment, runtime frequency, production-safety result, universal Evidence truth, or release readiness. A finite synthetic corpus may expose a defect or falsify a bounded claim; it cannot establish those broader claims.
