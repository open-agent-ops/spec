...
Независимо версионируемый профиль `agent-ops.evidence-acquisition@1.0.0` задаёт границу этапа 1 для сбора доказательств в [Приложении H](#приложение-h-профиль-сбора-доказательств-evidence-acquisition-100). Он разделяет требования, попытки сбора, артефакты, утверждения и пакеты, устанавливает независимые оси качества доказательств и сохраняет видимость сбоев сбора. В этой редакции профиль не вводит новые поля JSON и не меняет смысл Evidence Bundle v1 (пакета доказательств версии 1).
...
Граница доверия ограниченного профиля C4 на этапе 1 приведена в Приложении G. Безопасность допуска, знание об эффекте, успешность Outcome (результата) и последующее восстановление являются разными заявлениями. Безопасная остановка не устанавливает остальные три свойства, а атомарная удалённая авторизация не заявляется без взаимодействующего шлюза или backend (серверной стороны исполнения), применяющего решение в объявленной точке допуска.
...
Для `agent-ops.c4-human-approved-apply@1.0.0` этап 1 публикует допущения и границы заявлений, а настоящая исходная ревизия добавляет ограниченную исполняемую спецификацию этапа 2. Никакую runtime evaluation (оценку работающего исполнения) нельзя представлять как соответствие профилю до принятия точного кандидата этапа 2 и последующих схем, валидаторов отношений и негативных трасс этапа 3. Успешная ограниченная проверка модели не подменяет безопасность полномочий, измерение фактического эффекта, успешность Outcome (результата), полноту наблюдения или доказательства боевой среды.
...
Редакция v0.5.0, исходная ревизия `aom-04-r3`, заменяет текстовый кандидат v0.4.0 и публикует границы этапа 1 и кандидат этапа 2 для двух независимо версионируемых профилей: ограниченного C4 Human-approved Apply (применения после одобрения человеком) и Evidence Acquisition (сбора доказательств). Здесь нормативным кандидатом являются их стабильные идентичности, охват, явные нецели, границы допущений и заявлений, доверенные вычислительные базы, границы общих причин отказа, отдельные ограниченные исполняемые модели, независимо версионируемые каталоги отношений и небольшой семантический интерфейс. Эта редакция не предоставляет схемы или валидаторы этапа 3, не реализует среду исполнения, не сообщает результат оценки работающей среды и не устанавливает безопасность боевой среды. Предыдущие ревью и сохранённые байты PDF v0.4.0 не одобряют и не отображают этот преемник. Для выпуска нужны новые привязанные к точной цели ревью владельца, двуязычное ревью и доказательства детерминированного Build/Test PDF. Само наличие в репозитории не делает редакцию выпущенной.
...
Стабильный публичный идентификатор этого профиля — `agent-ops.c4-human-approved-apply@1.0.0`. Настоящее приложение является нормативным контрактом этапа 1 для его границы, допущений, словаря заявлений и доверенной вычислительной базы и вместе с версионируемыми артефактами в `formal/` задаёт кандидат этапа 2. Оно намеренно не определяет схемы и валидаторы отношений этапа 3. Реализация НЕ ДОЛЖНА заявлять runtime-conformance (соответствие работающего исполнения) этому профилю только на основании настоящего приложения или ограниченной модели.
...
Этап 1 устанавливает этот словарь и приведённые ниже допущения. G.7 предоставляет ограниченные определения этапа 2; они становятся гарантиями работающей среды, которые можно заявлять, лишь после принятия точного кандидата этапа 2 и обязательств соответствия этапа 3 и предоставления реализацией актуальных доказательств для точной цели.
...
| --- | --- | --- | --- |
| C4-G01 Безопасность допуска точного пакета | безопасность | C4-A02-A04, C4-A08-A10, C4-A18-A19; центр идентичности и политик, подписант, канонизатор, шлюз или серверная сторона, источники времени и версий | Только взаимодействующие шлюз или серверная сторона и точная типизированная операция; ограничена C4-R01-C4-R06 и P1/P2/P6; пока не является заявлением работающей среды. |
| C4-G02 Безопасность конфликтов и устаревших писателей | безопасность | C4-A02, C4-A04-A07, C4-A13; хранилище резервирований и отсечения и взаимодействующая серверная сторона | Только объявленный домен конфликтов и взаимодействующие писатели; ограничена C4-R04/C4-R05 и P4. |
| C4-G03 Безопасность повтора | безопасность | C4-A05-A07, C4-A11, C4-A15; серверная сторона, журнал, наблюдатель и авторизованный механизм сверки | Только зарегистрированное семейство операций; C4-R08 требует ключ backend, полное отрицательное Evidence (доказательство) или авторизованную сверку; одной компенсации недостаточно. |
| C4-G04 Непрерывность полномочий и бюджета | безопасность | C4-A06-A10, C4-A17-A18; журнал, хранилище резервирований, центр идентичности и политик, канонизатор, источники времени и версий | Переходы контрольной точки, передачи, перезапуска и истечения аренды; ограничена C4-R07/C4-R12 и P3. |
| C4-G05 Честное знание об эффекте | эпистемическая безопасность | C4-A06, C4-A11, C4-A13; журнал и наблюдатель эффектов | Только объявленная поверхность наблюдения; C4-R09 и CEI-R01-CEI-R07 требуют сохранять `unknown` при неполном охвате; ограничена P5. |
| C4-G06 Целостность классификации Outcome (результата) | эпистемическая безопасность | C4-A06, C4-A12-A13; журнал и оракул Outcome (результата) | Только исходное Intent (намерение) и заявленный охват оракула; ограничена C4-R10 и P7. |
| C4-G07 Итоговое разрешение обязательств | живучесть | C4-A01, C4-A05-A07, C4-A11-A15, C4-A17; все требуемые сервисы плоскости управления и путь эскалации человеку | C4-R11/C4-R13 и L1 проверяют наличие ограниченного авторизованного пути разрешения при доступности и обратимости; неограниченная живучесть не доказана. |

### G.7. Исполняемая модель и каталог отношений этапа 2

Этап 2 задаёт две композиционные исполняемые версионированные модели: модель полномочий и эффекта C4 в `formal/c4.go` и модель утверждений и достаточности Evidence Acquisition (сбора доказательств) в `formal/evidence.go`. Их соединяет `agent-ops.c4-evidence-interface@1.0.0` без слияния доменов полномочий. Независимо версионируемый каталог C4 — `agent-ops.c4-relations@1.0.0`; каждое отношение называет источник полномочий или авторитетного факта, момент проверки и исход отказа. Состояние C4 включает канонические свёртку и версию пакета, UID цели и критичные для решения версии, эпохи утверждения и политики, ограниченный временной интервал, события утверждения и запуска человеком, бюджет и число израсходованных попыток, домен конфликтов, резервирование и метку отсечения, состояние допуска, знание об эффекте, состояние Outcome (результата), обязательства сверки или компенсации и происхождение контрольной точки.

Действия модели включают создание пакета, утверждение человеком, отзыв, запуск человеком, резервирование, допуск либо запрет в точке линеаризации шлюза или серверной стороны, наблюдение эффекта, потерю ответа, запрос повтора, истечение аренды, отклонение по метке отсечения, сверку, компенсацию, оценку Outcome (результата), контрольную точку, перезапуск и окончательное освобождение. Модель разделяет безопасность и ограниченный анализ наличия пути разрешения, явно представляет недоступные или снятые допущения, использует именованные события вместо неограниченного ожидания по настенным часам и сохраняет `unknown` как самостоятельное состояние.
...
Исполняемая проверка использует закреплённый Go 1.26.8, детерминированный полный обход до глубины 8, не использует случайное начальное число и применяет конечные наборы событий из `formal/check.go`. На этом кандидате она посещает 1233 состояния C4 по 6014 принятым переходам и 332 состояния Evidence по 2325 принятым переходам. `formal/counterexamples.json` сохраняет минимизированные трассы, связанные с именованными снятыми допущениями, включая три обязательные совместные трассы. Ограниченная модель может опровергнуть универсальное заявление; успешная ограниченная проверка не доказывает безопасность боевой среды. Точное принятие остаётся решением владельца по проверенному кандидату.

### G.8. Граница этапа и следующее решение

Этап 1 завершён. Настоящая исходная ревизия предоставляет точный кандидат этапа 2 для темпоральной семантики и семантики полномочий, сбора, утверждений и достаточности. Этап 3 может начаться только после принятия ответственным владельцем обеих исполняемых моделей, `agent-ops.c4-evidence-interface@1.0.0` и обоих независимо версионируемых каталогов отношений на точной проверенной цели. Рабочий шлюз, журнал, наблюдатель, сборщики, адаптеры серверной стороны, оценка исполнения, боевая эксплуатация, платная оценка, исследование с участием людей и выпуск требуют отдельных применимых решений и доказательств.

## Приложение H. Профиль сбора доказательств Evidence Acquisition 1.0.0

### H.1. Статус, идентичность, охват и нецели

Стабильный публичный идентификатор этого профиля — `agent-ops.evidence-acquisition@1.0.0`. Настоящее приложение является его нормативным контрактом этапа 1 для границ сбора, осей качества, семантики отказов, доверенной вычислительной базы и обработки общих причин отказа и вместе с `formal/` задаёт кандидат отношений и переходов этапа 2. Оно намеренно не задаёт поля JSON, схемы или публичный контракт валидатора этапа 3, реализацию рабочей среды либо заявление о соответствии боевой среды.
...
| Независимость | Какие источники, учётные данные, сборщики, анализаторы, хранилища, часы, пути, модели или запросы к модели являются общими? |
...
| Валидатор отношений | Будущий компонент этапа 3, проверяющий принятые отношения этапа 2; не может вывести отсутствующие факты или полномочия. |
...
Профили остаются независимыми. Доказательство никогда не даёт утверждение или полномочия на исполнение, а полномочия допуска C4 никогда не доказывают истинность, полноту, актуальность или достаточность доказательства. Правомерно допущенная операция может иметь `unknown` эффект или Outcome (результат); хорошо обоснованное наблюдение может описывать неавторизованную операцию, не разрешая её. `agent-ops.c4-evidence-interface@1.0.0` переносит применимость требования и получателя, точную цель и эпоху, актуальность в момент использования с версиями, критичными для решения, охват наблюдения и способность обнаружения, диспозицию утверждения, достаточность для конкретного получателя, связывание происхождения и раскрытие общих причин отказа. Его идентификаторы отношений — CEI-R01-CEI-R07. Сериализация в записи этапа 3 остаётся зарезервированной.

### H.7. Границы общих причин отказа и контрпримеры

Запись реализации ДОЛЖНА перечислять общие источники, учётные данные, сборщики, анализаторы, хранилища, часы, сетевые пути, модели, запросы к моделям, канонизаторы, сервисы идентичности и API цели. Два утверждения, полученные через одну дефектную зависимость, не являются независимыми доказательствами этой зависимости. Различающиеся имена процесса, модели или запроса не устанавливают независимость, если общими остаются полномочия, данные, учётные данные или путь наблюдения.

Этап 2 сохраняет минимизированные контрпримеры как минимум для следующих случаев: артефакт с действующим TTL после изменения версии, критичной для решения; вывод `not_applied` из неполного охвата; успешный транспорт с усечённым или неразбираемым содержимым; запечатанный пакет с неавторитетным источником; два якобы независимых утверждения с одним ложным источником; правильное наблюдение эффекта, ошибочно принятое за успешность Outcome (результата); и действительные полномочия C4, ошибочно принятые за истинность доказательства. Они являются исполняемыми записями `formal/counterexamples.json`.
...
### H.8. Граница этапа и следующее решение

Настоящий кандидат задаёт исполняемую модель Evidence Acquisition (сбора доказательств), связывает её через `agent-ops.c4-evidence-interface@1.0.0` с отдельной моделью C4 и публикует `agent-ops.evidence-relations@1.0.0`. Запись детерминированной проверки и сохранённые контрпримеры заданы в G.7 и `formal/README.md`. Схемы и публичные валидаторы этапа 3 могут создаваться только после принятия обеих моделей, интерфейса и обоих каталогов отношений на точной проверенной цели. Существующий Evidence Bundle v1 остаётся неизменным до явного принятия несовместимого преемника и миграции.
