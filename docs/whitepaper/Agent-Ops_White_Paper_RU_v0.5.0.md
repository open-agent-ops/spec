...
**Публичный нормативный кандидат v0.5.0 | исходная ревизия aom-04-r4 | приоритет английской версии**
...
Независимо версионируемый профиль `agent-ops.evidence-acquisition@1.0.0` задаёт границу сбора доказательств в [Приложении H](#приложение-h-профиль-сбора-доказательств-evidence-acquisition-100). Он разделяет требования, попытки сбора, артефакты, утверждения и пакеты, устанавливает независимые оси качества доказательств и сохраняет видимость сбоев сбора. В этой редакции профиль не вводит новые поля JSON и не меняет смысл Evidence Bundle v1 (пакета доказательств версии 1).
...
Редакция v0.5.0, исходная ревизия `aom-04-r4`, заменяет текстовый кандидат v0.4.0 и публикует два независимо версионируемых профиля: ограниченный C4 Human-approved Apply (применение после одобрения человеком) и Evidence Acquisition (сбор доказательств). Здесь нормативным кандидатом являются их стабильные идентичности, охват, явные нецели, границы допущений и заявлений, доверенные вычислительные базы, границы общих причин отказа, отдельные ограниченные исполняемые формальные модели состояний и переходов, независимо версионируемые каталоги отношений и небольшой семантический интерфейс. Эта редакция также публикует ограниченный срез реализации C4: шесть схем записей, `agent-ops.relation-validation@1.0.0`, положительные, схемно-негативные и межзаписно-негативные фикстуры. Она не реализует среду исполнения, не предоставляет полный валидатор жизненного цикла, не сообщает результат оценки работающей среды и не устанавливает безопасность боевой среды. Предыдущие ревью и сохранённые байты PDF v0.4.0 не одобряют и не отображают этот преемник. Для выпуска нужны новые привязанные к точной цели ревью владельца, двуязычное ревью и доказательства детерминированного Build/Test PDF. Само наличие в репозитории не делает редакцию выпущенной.
...
Стабильный публичный идентификатор этого профиля — `agent-ops.evidence-acquisition@1.0.0`. Настоящее приложение является его нормативным контрактом границ сбора, осей качества, семантики отказов, доверенной вычислительной базы и обработки общих причин отказа и вместе с `formal/` задаёт исполняемый кандидат отношений и переходов. Оно намеренно не задаёт поля JSON, схемы или публичный контракт валидатора реализации, реализацию рабочей среды либо заявление о соответствии боевой среды.
...
| `EvidenceBundle` | Адресуемый по содержимому пакет ссылок и происхождения. Упаковка или запечатывание не создают полномочия или качество доказательств. Существующая семантика Bundle v1 (пакета версии 1) не меняется. |
...
Настоящий кандидат задаёт исполняемую формальную модель состояний и переходов Evidence Acquisition (сбора доказательств), связывает её через `agent-ops.c4-evidence-interface@1.0.0` с отдельной формальной моделью состояний и переходов C4 и публикует `agent-ops.evidence-relations@1.0.0`. Запись детерминированной проверки и сохранённые контрпримеры заданы в G.7 и `formal/README.md`. Ограниченный срез записей C4 проверяет факты Evidence (доказательств) в точке их использования C4, но не предоставляет самостоятельное семейство схем Evidence Acquisition или универсальный валидатор отношений Evidence. Существующий Evidence Bundle v1 остаётся неизменным до явного принятия несовместимого преемника и миграции.
