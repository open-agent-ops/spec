...
**Публичный нормативный кандидат v0.5.0 | исходная ревизия aom-04-r2 | приоритет английской версии**
...
Для `agent-ops.c4-human-approved-apply@1.0.0` этап 1 публикует только допущения и информативный план формальной модели. Никакую runtime evaluation (оценку работающего исполнения) нельзя представлять как соответствие профилю до принятия темпоральной модели этапа 2 и схем, валидаторов отношений и негативных трасс этапа 3. Доля прохождения контрольных ворот не подменяет безопасность полномочий, измерение фактического эффекта, успешность Outcome (результата) или полноту наблюдения.
...
Редакция v0.5.0, исходная ревизия `aom-04-r2`, заменяет текстовый кандидат v0.4.0 и публикует этап 1 двух независимо версионируемых профилей: ограниченного C4 Human-approved Apply (применения после одобрения человеком) и Evidence Acquisition (сбора доказательств). Здесь нормативно заданы их стабильные идентичности, охват, явные нецели, границы допущений и заявлений, доверенные вычислительные базы, границы общих причин отказа и согласованный план формальных моделей. Эта редакция не задаёт модели или каталоги отношений этапа 2, не предоставляет схемы или валидаторы этапа 3, не реализует среду исполнения, не сообщает результат оценки и не устанавливает безопасность боевой среды. Предыдущие ревью и сохранённые байты PDF v0.4.0 не одобряют и не отображают этот преемник. Для выпуска нужны новые привязанные к точной цели ревью владельца, двуязычное ревью и доказательства детерминированного Build/Test PDF. Само наличие в репозитории не делает редакцию выпущенной.
...
Стабильный публичный идентификатор этого профиля — `agent-ops.c4-human-approved-apply@1.0.0`. Настоящее приложение является нормативным контрактом этапа 1 для его границы, допущений, словаря заявлений и доверенной вычислительной базы. Оно намеренно не определяет темпоральную машину состояний этапа 2 или схемы и валидаторы отношений этапа 3. Реализация НЕ ДОЛЖНА заявлять соответствие этому профилю только на основании настоящего приложения.
...
Этап 1 устанавливает только этот словарь и приведённые ниже допущения. Предлагаемые свойства из G.7 становятся гарантиями, которые можно заявлять, лишь после принятия их определений этапа 2 и обязательств соответствия этапа 3 и предоставления реализацией актуальных доказательств для точной цели.
...
| --- | --- | --- | --- |
| C4-G01 Безопасность допуска точного пакета | безопасность | C4-A02-A04, C4-A08-A10, C4-A18-A19; центр идентичности и политик, подписант, канонизатор, шлюз или серверная сторона, источники времени и версий | Только взаимодействующие шлюз или серверная сторона и точная типизированная операция; определение оставлено этапу 2, заявление пока невозможно. |
| C4-G02 Безопасность конфликтов и устаревших писателей | безопасность | C4-A02, C4-A04-A07, C4-A13; хранилище резервирований и отсечения и взаимодействующая серверная сторона | Только объявленный домен конфликтов и взаимодействующие писатели; определение оставлено этапу 2. |
| C4-G03 Безопасность повтора | безопасность | C4-A05-A07, C4-A11, C4-A15; серверная сторона, журнал, наблюдатель и авторизованный механизм сверки | Только зарегистрированное семейство операций; одной компенсации недостаточно; определение оставлено этапу 2. |
| C4-G04 Непрерывность полномочий и бюджета | безопасность | C4-A06-A10, C4-A17-A18; журнал, хранилище резервирований, центр идентичности и политик, канонизатор, источники времени и версий | Переходы контрольной точки, передачи, перезапуска и истечения аренды; определение оставлено этапу 2. |
| C4-G05 Честное знание об эффекте | эпистемическая безопасность | C4-A06, C4-A11, C4-A13; журнал и наблюдатель эффектов | Только объявленная поверхность наблюдения; неполный охват требует `unknown`; определение оставлено этапу 2. |
| C4-G06 Целостность классификации Outcome (результата) | эпистемическая безопасность | C4-A06, C4-A12-A13; журнал и оракул Outcome (результата) | Только исходное Intent (намерение) и заявленный охват оракула; определение оставлено этапу 2. |
| C4-G07 Итоговое разрешение обязательств | живучесть | C4-A01, C4-A05-A07, C4-A11-A15, C4-A17; все требуемые сервисы плоскости управления и путь эскалации человеку | Только при выполнении заявленных допущений доступности, обратимости, справедливости планирования и ответа; план оставлен этапу 2. |

### G.7. Информативный план формальной модели

До фиксации схем этап 2 должен ввести две композиционные исполняемые версионированные модели: описанную здесь модель полномочий и эффекта C4 и описанную в Приложении H модель утверждений и достаточности Evidence Acquisition (сбора доказательств). Их соединяет небольшой версионируемый семантический интерфейс без слияния доменов полномочий. Планируемое состояние C4 включает канонические свёртку и версию пакета, UID цели и критичные для решения версии, эпохи утверждения и политики, ограниченный временной интервал, события утверждения и запуска человеком, бюджет и число израсходованных попыток, ключ идемпотентности, домен конфликтов, резервирование и метку отсечения, состояние допуска, знание об эффекте, состояние Outcome (результата), обязательства сверки или компенсации, охват наблюдателя и происхождение контрольной точки.

Планируемые действия включают создание пакета, утверждение человеком, отзыв, запуск человеком, резервирование, допуск либо запрет в точке линеаризации шлюза или серверной стороны, наблюдение эффекта, потерю ответа, запрос повтора, истечение аренды, отклонение по метке отсечения, сверку, компенсацию, оценку Outcome (результата), контрольную точку, перезапуск и окончательное освобождение. Модель разделит безопасность и живучесть, включит явные абстракции недоступности и компрометированной границы, будет использовать именованные барьеры событий вместо неограниченного ожидания по настенным часам и сохранит `unknown` как самостоятельное состояние.
...
Запись принятия этапа 2 должна назвать средства проверки обеих моделей или исполняемые спецификации, точные версии, конфигурации, границы, допущения справедливости, начальные числа для генерации, если они применяются, проверенные свойства, полные выводы и сохранённые минимизированные контрпримеры. Она также должна указать версию интерфейса и независимо версионируемые каталоги отношений C4 и Evidence (доказательств). Ограниченная модель может опровергнуть универсальное заявление; успешная ограниченная проверка не доказывает безопасность боевой среды.

### G.8. Граница этапа и следующее решение

Этап 1 завершён только после принятия ответственным владельцем точных границ профилей C4 и Evidence (доказательств), каталогов допущений, таблиц доверенной вычислительной базы, словарей заявлений, обработки общих причин отказа, нецелей и согласованного плана моделей на текущем кандидате. После этого этап 2 может определить темпоральную семантику, семантику полномочий, сбора, утверждений и достаточности; их НЕЛЬЗЯ выводить из настоящих приложений. Этап 3 может начаться только после принятия обеих моделей этапа 2, их небольшого версионируемого интерфейса и обоих независимо версионируемых каталогов отношений. Рабочий шлюз, журнал, наблюдатель, сборщики, адаптеры серверной стороны, оценка исполнения, боевая эксплуатация, платная оценка, исследование с участием людей и выпуск требуют отдельных применимых решений и доказательств.

## Приложение H. Профиль сбора доказательств Evidence Acquisition 1.0.0

### H.1. Статус, идентичность, охват и нецели

Стабильный публичный идентификатор этого профиля — `agent-ops.evidence-acquisition@1.0.0`. Настоящее приложение является его нормативным контрактом этапа 1 для границ сбора, осей качества, семантики отказов, доверенной вычислительной базы, обработки общих причин отказа и интерфейса с ограниченным профилем C4. Оно намеренно не задаёт идентификаторы отношений или темпоральную машину состояний этапа 2, поля JSON, схемы или публичный контракт валидатора этапа 3, реализацию рабочей среды либо заявление о соответствии боевой среды.
...
Профили остаются независимыми. Доказательство никогда не даёт утверждение или полномочия на исполнение, а полномочия допуска C4 никогда не доказывают истинность, полноту, актуальность или достаточность доказательства. Правомерно допущенная операция может иметь `unknown` эффект или Outcome (результат); хорошо обоснованное наблюдение может описывать неавторизованную операцию, не разрешая её. Интерфейс этапа 2 будет переносить требование применимости, получателя, точную цель и эпоху, актуальность в момент использования с версиями, критичными для решения, охват наблюдения и способность обнаружения, диспозицию утверждения, достаточность для конкретного получателя, а также ссылки на происхождение и общие причины отказа. Этот абзац задаёт только семантику и резервирует все идентификаторы отношений и сериализации.
...
Этап 2 ДОЛЖЕН сохранить минимизированные контрпримеры как минимум для следующих случаев: артефакт с действующим TTL после изменения версии, критичной для решения; вывод `not_applied` из неполного охвата; успешный транспорт с усечённым или неразбираемым содержимым; запечатанный пакет с неавторитетным источником; два якобы независимых утверждения с одним ложным источником; правильное наблюдение эффекта, ошибочно принятое за успешность Outcome (результата); и действительные полномочия C4, ошибочно принятые за истинность доказательства.

### H.8. Граница этапа и следующее решение

Этап 2 должен определить исполняемую версионированную модель Evidence Acquisition (сбора доказательств), связать её небольшим семантическим интерфейсом с отдельной моделью C4 и опубликовать независимо версионируемый каталог отношений Evidence (доказательств). Запись принятия должна указать точные инструменты, версии, конфигурации, границы, свойства, начальные числа, если применимы, полные выводы и сохранённые минимизированные контрпримеры. Схемы и публичные валидаторы этапа 3 могут создаваться только после принятия обеих моделей, интерфейса и обоих каталогов отношений. Существующий Evidence Bundle v1 остаётся неизменным до явного принятия несовместимого преемника и миграции.
