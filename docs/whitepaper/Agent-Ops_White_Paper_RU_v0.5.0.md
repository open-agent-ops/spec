...
**Публичный нормативный кандидат v0.5.0 | исходная ревизия aom-04-r3 | приоритет английской версии**
...
Для `agent-ops.c4-human-approved-apply@1.0.0` профиль публикует допущения и границы заявлений, а настоящая исходная ревизия включает ограниченную исполняемую спецификацию. Никакую runtime evaluation (оценку работающего исполнения) нельзя представлять как соответствие профилю до принятия точного кандидата, требуемых схем, валидаторов отношений и негативных трасс. Успешная ограниченная проверка формальной модели состояний и переходов не подменяет безопасность полномочий, измерение фактического эффекта, успешность Outcome (результата), полноту наблюдения или доказательства боевой среды.
...
Редакция v0.5.0, исходная ревизия `aom-04-r3`, заменяет текстовый кандидат v0.4.0 и публикует два независимо версионируемых профиля: ограниченный C4 Human-approved Apply (применение после одобрения человеком) и Evidence Acquisition (сбор доказательств). Здесь нормативным кандидатом являются их стабильные идентичности, охват, явные нецели, границы допущений и заявлений, доверенные вычислительные базы, границы общих причин отказа, отдельные ограниченные исполняемые формальные модели состояний и переходов, независимо версионируемые каталоги отношений и небольшой семантический интерфейс. Эта редакция не предоставляет схемы реализации или публичные валидаторы отношений, не реализует среду исполнения, не сообщает результат оценки работающей среды и не устанавливает безопасность боевой среды. Предыдущие ревью и сохранённые байты PDF v0.4.0 не одобряют и не отображают этот преемник. Для выпуска нужны новые привязанные к точной цели ревью владельца, двуязычное ревью и доказательства детерминированного Build/Test PDF. Само наличие в репозитории не делает редакцию выпущенной.
...
Стабильный публичный идентификатор этого профиля — `agent-ops.c4-human-approved-apply@1.0.0`. Настоящее приложение является нормативным контрактом его границы, допущений, словаря заявлений и доверенной вычислительной базы и вместе с версионируемыми артефактами в `formal/` задаёт ограниченный исполняемый кандидат. Оно намеренно не определяет схемы реализации и публичные валидаторы отношений. Реализация НЕ ДОЛЖНА заявлять runtime-conformance (соответствие работающего исполнения) этому профилю только на основании настоящего приложения или ограниченной формальной модели состояний и переходов.
...
### G.8. Граница кандидата и следующее решение

Настоящая исходная ревизия предоставляет точный кандидат темпоральной семантики и семантики полномочий, сбора, утверждений и достаточности. Работа над схемами реализации и публичными валидаторами отношений может начаться только после принятия ответственным владельцем обеих исполняемых формальных моделей состояний и переходов, `agent-ops.c4-evidence-interface@1.0.0` и обоих независимо версионируемых каталогов отношений на точной проверенной цели. Рабочий шлюз, журнал, наблюдатель, сборщики, адаптеры серверной стороны, оценка исполнения, боевая эксплуатация, платная оценка, исследование с участием людей и выпуск требуют отдельных применимых решений и доказательств.
...
| Валидатор отношений | Будущий компонент, проверяющий принятые отношения; не может вывести отсутствующие факты или полномочия. |
...
Профили остаются независимыми. Доказательство никогда не даёт утверждение или полномочия на исполнение, а полномочия допуска C4 никогда не доказывают истинность, полноту, актуальность или достаточность доказательства. Правомерно допущенная операция может иметь `unknown` эффект или Outcome (результат); хорошо обоснованное наблюдение может описывать неавторизованную операцию, не разрешая её. `agent-ops.c4-evidence-interface@1.0.0` переносит применимость требования и получателя, точную цель и эпоху, актуальность в момент использования с версиями, критичными для решения, охват наблюдения и способность обнаружения, диспозицию утверждения, достаточность для конкретного получателя, связывание происхождения и раскрытие общих причин отказа. Эти интерфейсные отношения (`CEI-R01`-`CEI-R07`) по отдельности сопоставлены со смыслом в указателе отношений G.7. Сериализация в машиночитаемые записи реализации остаётся зарезервированной.
...
### H.8. Граница кандидата и следующее решение

Настоящий кандидат задаёт исполняемую формальную модель состояний и переходов Evidence Acquisition (сбора доказательств), связывает её через `agent-ops.c4-evidence-interface@1.0.0` с отдельной формальной моделью состояний и переходов C4 и публикует `agent-ops.evidence-relations@1.0.0`. Запись детерминированной проверки и сохранённые контрпримеры заданы в G.7 и `formal/README.md`. Схемы реализации и публичные валидаторы могут создаваться только после принятия обеих формальных моделей, интерфейса и обоих каталогов отношений на точной проверенной цели. Существующий Evidence Bundle v1 остаётся неизменным до явного принятия несовместимого преемника и миграции.
