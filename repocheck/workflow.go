...
func WorkflowModel(lock ToolLock, pins map[string]string, release bool) map[string]any {
	...
	mirror := map[string]any{"name": "AOM / mirror", "runs-on": "ubuntu-24.04", "permissions": read, "timeout-minutes": 30, "environment": "public-mirror", "needs": []any{"publish"}, "env": map[string]any{"AOM_MIRROR_REPOSITORY": "${{ vars.AOM_MIRROR_REPOSITORY }}"}, "steps": []any{checkout("${{ github.sha }}"), map[string]any{"uses": pins["actions/download-artifact"], "with": map[string]any{"name": "release-candidate-${{ github.run_id }}", "path": ".aom-release/"}}, map[string]any{"env": map[string]any{"AOM_MIRROR_TOKEN": "${{ secrets.AOM_MIRROR_TOKEN }}", "AOM_GITHUB_TOKEN": "${{ github.token }}"}, "run": "chmod 700 .aom-release/spec-release\n.aom-release/spec-release mirror > .aom-release/mirror-receipt.json"}}}
